...
function App() {
  ...
  useEffect(() => {
    fetch("http://localhost:3030/jsonstore/todos").then((response) =>
      response.json().then((data) => {
        const result = Object.keys(data).map((id) => ({ id: id -1, ...data[id] }));
        setTodos(result);
      })
    );
  }, []);
  ...
  const addNewTodo = () => {
    ...
    if(isNaN(lastId)){
     setTodos(oldTodos => [ ...oldTodos,{id: 0, text : inputText , isCompleted: false}])
    } else {
      setTodos(oldTodos => [ ...oldTodos,{id: lastId + 1, text : inputText , isCompleted: false}])
    }
        ...
        <section className="todo-list-container">
          ...
          <div className="table-wrapper">
            ...
            <table className="table">
              ...
              <TodoList todos={todos} toggleStatus={toggleStatus}  onDelete={onDelete}/>
            </table>
          </div>
        </section>
      </main>
